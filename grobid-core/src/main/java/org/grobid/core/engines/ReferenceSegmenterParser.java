...
public class ReferenceSegmenterParser extends AbstractParser implements ReferenceSegmenter {
    ...
    public List<LabeledReferenceResult> extract(
        ...
        try {
            res = labelWithLongSequenceSupport(featureVector);
        } catch (Exception e) {
     ...
     */
    private String labelWithLongSequenceSupport(String featureVector) {
                    ...
                    for (int j = previousTransitionPos; j < transitionPos; j++) {
                        if (j == previousTransitionPos && previousTransitionPos != 0) {
                            ...
                            if (localLine.indexOf(TaggingLabels.GROBID_START_ENTITY_LABEL_PREFIX) == -1) {
                                localLine = localLine
                                        .replace("<label>", TaggingLabels.GROBID_START_ENTITY_LABEL_PREFIX + "<label>");
                            ...
                            if (localLine.indexOf(TaggingLabels.GROBID_START_ENTITY_LABEL_PREFIX) != -1) {
                                localLine = localLine
                                        .replace(TaggingLabels.GROBID_START_ENTITY_LABEL_PREFIX + "<label>", "<label>");
                                localLine = localLine.replace(
                                        TaggingLabels.GROBID_START_ENTITY_LABEL_PREFIX + "<reference>",
                                        "<reference>");
                            }
    ...
    private List<LabeledReferenceResult> getExtractionResult(
            List<LayoutToken> tokenizations,
            List<Triple<String, String, String>> labeled) {
        ...
        Function<LabeledTokensContainer, Void> function = new Function<LabeledTokensContainer, Void>() {
            @Override
            public Void apply(LabeledTokensContainer container) {
                    ...
                    if (reference.length() != 0) {
                        resultList.add(
                                new LabeledReferenceResult(
                                        ...
                                        reference.toString().trim(), Lists.newArrayList(referenceTokens),
    ...
    static public Pair<String, List<LayoutToken>> getReferencesSectionFeatured(
            Document doc,
            SortedSet<DocumentPiece> referencesParts) {
